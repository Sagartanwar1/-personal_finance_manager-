// Simple demo that calls the FastAPI backend and shows transactions.
// Edit backend URL if not using docker-compose mapped ports.
...
async function fetchTx() {
  ...
  try {
    ...
    tbody.innerHTML = "";
    if (tx.length === 0) {
      msg.textContent = "No transactions found. Use the backend API to POST /transactions.";
      table.style.display = "none";
    } else {
      tx.forEach(t => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td>${t.id}</td><td>${t.type}</td><td>${t.category}</td><td>${t.amount}</td><td>${t.date}</td>`;
    ...
    msg.textContent = "Error fetching transactions: " + e.message;
  } finally {
...
async function tryPostTransaction(payload){
  // if backend available, POST to /transactions
  try{
